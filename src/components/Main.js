...
const Main = () => {
  ...
  const handleSetAdditional = i => {
    if (additional.includes(i))
      setAdditional(additional.filter(item => item !== i));
  ...
  const pages = [
    {
      page: (
        <ChooseFirst
          ...
          setCategory={c => setSelectedCategory(c)}
          content="Valitse"
        />
      ),
      index: 1,
      title: "What kind of food do you want?"
    },
    {
      page: <Constraints setDiet={setDiet} setAllergines={setAllergines} />,
      index: 2,
      title: "Do you have any allergies or diets?"
    },
    {
      page: (
        <AdditionalProducts
          resetAdditional={resetAdditional}
          addAdditional={handleSetAdditional}
        />
      ),
      index: 3,
    ...
    {
      page: <FinalPage setServings={setServings} additional={additional} />,
      index: 4,
      title: "Order details"
    }
  ];
  ...
  return (
    <>
      ...
      {landed ? (
        orderDone ? (
          <OrderDone servings={servings} recipe={prediction} />
        ) : (
          <Navigator
            selectedCategory={selectedCategory}
            pages={pages}
            onConfirm={() => setOrderDone(true)}
          />
        )
      ) : (
